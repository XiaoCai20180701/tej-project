...
<script>
  ...
  import { notPassVendorTable, auditType } from '@/api/tableData'
  ...
  export default {
    ...
    mounted(){
      this.columnsData = notPassVendorTable
      this.getList()
    },
    methods: {
      ...
      getList(){
        postVendorList({
          ...
          .catch(err => {
            this.$Message.error('获取厂商未通过列表失败',err)
          })
      },
    }
  }
